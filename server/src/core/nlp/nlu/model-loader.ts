  ...
  public loadNLPModels(): Promise<[void, void, void]> {
    return Promise.all([
      this.loadGlobalResolversModel(
        path.join(MODELS_PATH, 'neon-global-resolvers-model.nlp')
      ),
      this.loadSkillsResolversModel(
        path.join(MODELS_PATH, 'neon-skills-resolvers-model.nlp')
      ),
      this.loadMainModel(path.join(MODELS_PATH, 'neon-main-model.nlp'))
    ])
  }
