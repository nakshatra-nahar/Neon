...
const skillActionTypes = [
  ...
  Type.Literal('dialog', {
    description:
      "Action that don't need code to run. neon actually just answers without any business logic."
  })
]
  ...
  Type.Object({
      ...
      Type.Object(
        {
          ...
          loop: Type.Optional(
            Type.Object(
              ...
              },
              {
                additionalProperties: false,
                description:
                  'The action loop is a concept to keep neon triggering the same skill action until the logic of the skill breaks the loop according to new utterances content.'
              }
            )
          ),
          ...
          utterance_samples: Type.Optional(
            Type.Array(Type.String(), {
              description:
                'Utterance samples are used by the NLU (Natural Language Understanding) to train the skill. They are examples of what neon owners can say to trigger the skill action.'
            })
          ),
          ...
          suggestions: Type.Optional(
            Type.Array(Type.String(), {
              description:
                'Suggestions are a simple way to suggest neon owners what can be answered next.'
            })
          ),
          slots: Type.Optional(
            Type.Array(
              Type.Object(
                ...
                },
                {
                  additionalProperties: false,
                  description:
                    'A slot expects a type of data called "item", and makes use of questions to let neon owners knows what data they need to provide.'
                }
              ),
              {
                description:
                  'Depending on how skill developers wants to design their skill, they have the possibility to ask for more information before to get to the meat of the skill. In this way, neon can gather these information to operate the skill in a complete manner. These information are called "slots".'
              }
            )
          ),
    ...
    resolvers: Type.Optional(
      Type.Record(
        ...
        {
          description:
            'You can see resolvers as utterance samples that are converted (resolved) to a value of your choice. They are very handy when skills expect specific utterances and then according to these utterances attribute a value that can be handled by the skill. If a skill action expects to receive a resolver, then neon will convert the value for you and this value will be usable from the skill action code. Any value can be passed to resolvers which allow a large possibilities of usages.'
        }
      )
    )
  })
