...
export default class Client {
  ...
  async sendInitMessages() {
    for (let i = 0; i < INIT_MESSAGES.length; i++) {
      ...
      setTimeout(() => {
        this.chatbot.isTyping('neon', true)
      }, sendingDelay / typingFactorDelay)

      await new Promise((resolve) => setTimeout(resolve, sendingDelay))

      this.chatbot.receivedFrom('neon', message)
      this.chatbot.isTyping('neon', false)
    }
  }
  ...
  init(loader) {
    ...
    this.socket.on('answer', (data) => {
      this.chatbot.receivedFrom('neon', data)
    })
    ...
    this.socket.on('is-typing', (data) => {
      this.chatbot.isTyping('neon', data)
    })
  ...
  send(keyword) {
    if (this._input.value !== '') {
      ...
      this.chatbot.sendTo('neon', this._input.value)
