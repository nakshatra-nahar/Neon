/**
 * This file allows to run a separate node to detect the wake word "neon/Léon"
...
process.env.neon_HOST = process.env.neon_HOST || 'http://localhost'
process.env.neon_PORT = process.env.neon_PORT || 1337
const url = `${process.env.neon_HOST}:${process.env.neon_PORT}`
const socket = io(url)
const { argv } = process
...
;(async () => {
  try {
    ...
    models.add({
      file: `${__dirname}/models/neon-${lang}.pmdl`,
      sensitivity: '0.5',
      hotwords: `neon-${lang}`
    })
