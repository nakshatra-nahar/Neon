...
describe('server', () => {
  describe('init()', () => {
    test('uses default language if the given one is unsupported', async () => {
      ...
      process.env.neon_LANG = 'fake-lang'

      await server.init()
      expect(process.env.neon_LANG).toBe('en-US')
    })
  ...
  describe('listen()', () => {
    test('listens for request', async () => {
      ...
      await server.listen(process.env.neon_PORT)
      expect(
        console.log.mock.calls[1][1].indexOf(
          `${process.env.neon_HOST}:${process.env.neon_PORT}`
        )
      ).not.toEqual(-1)
    })
  })

  describe('handneonConnection()', () => {
