...
import { neon_VERSION, NODEJS_BRIDGE_VERSION } from '@bridge/constants'
...
export class Network {
  ...
  public async request<ResponseData = unknown, ResponseErrorData = unknown>(
    ...
    try {
      const response = await this.axios.request<string>({
        ...
        headers: {
          'User-Agent': `neon Personal Assistant ${neon_VERSION} - Node.js Bridge ${NODEJS_BRIDGE_VERSION}`,
          ...options.headers
        }
      })
  ...
  public async isNetworkAvailable(): Promise<boolean> {
    try {
      await dns.promises.resolve('getneon.ai')
