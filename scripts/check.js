...
;(async () => {
  try {
    ...
    const globalResolversNlpModelPath =
      'core/data/models/neon-global-resolvers-model.nlp'
    const skillsResolversNlpModelPath =
      'core/data/models/neon-skills-resolvers-model.nlp'
    const mainNlpModelPath = 'core/data/models/neon-main-model.nlp'
    const report = {
    ...
    /**
     * neon version checking
     */

    LogHelper.info('neon version')
    LogHelper.success(`${neon_VERSION}\n`)
    reportDataInput.neonVersion = neon_VERSION
    ...
    if (Math.round(freeRAMInGB) < MINIMUM_REQUIRED_RAM) {
      ...
      LogHelper.error(
        `Free RAM: ${freeRAMInGB} GB | Total RAM: ${totalRAMInGB} GB. neon needs at least ${MINIMUM_REQUIRED_RAM} GB of RAM`
      )
    } else {
      LogHelper.success(
    ...
    p.stdout.on('end', async () => {
      ...
      if (
        ...
        LogHelper.success('Hooray! neon can run correctly')
        LogHelper.info(
      ...
      try {
        const { data } = await axios.post('https://getneon.ai/api/report', {
