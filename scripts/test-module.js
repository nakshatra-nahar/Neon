...
;(async () => {
  ...
  try {
    ...
    const cmd = await command(
      `cross-env PIPENV_PIPFILE=bridges/python/src/Pipfile neon_NODE_ENV=testing jest --silent --config=./test/e2e/modules/e2e.modules.jest.json packages/${pkg}/test/${module}.spec.js && npm run train`,
      { shell: true }
    )
